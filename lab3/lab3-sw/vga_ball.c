...
static void read_pos(vga_ball_pos_t *pos)
{
	pos->x = ioread16(BALL_X(dev.virtbase));
	pos->y = ioread16(BALL_Y(dev.virtbase));
	dev.pos = *pos;
}

/*
 * Handle ioctl() calls from userspace:
 * Read or write ball coordinates.
...
{
	vga_ball_pos_t initial_pos = { 320, 240 };
	int ret;
